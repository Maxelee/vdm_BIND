...
def draw_figure(x,sample,conditioning,dataset):
    ...
    if plot:
        ...
        ax.flat[1].imshow(x[0, 0].squeeze().cpu(), cmap='cividis', vmin=-2, vmax=6)
        ax.flat[2].imshow(sample[0, 0].squeeze().cpu(), cmap='cividis', vmin=-2, vmax=6)
        ...
        ax.flat[3].hist(x[0, 0].cpu().numpy().flatten(), bins=np.linspace(-4, 4, 50), histtype='step', color='#4c4173', label="True DM", density=True)
        ax.flat[3].hist(sample[0, 0].cpu().numpy().flatten(), bins=np.linspace(-4, 4, 50), alpha=0.5, color='#4c4173', label="Sampled DM", density=True)
