...
from vdm.utils import draw_figure, load_checkpoint
...
# Re-export ConfigLoader and load_normalization_stats from canonical module
# to maintain backward compatibility for code importing from workflow_utils
...
class ModelManager:
    ...
    def detect_model_type(config, verbose=False):
        ...
        if config.best_ckpt and os.path.exists(config.best_ckpt):
            try:
                checkpoint = load_checkpoint(config.best_ckpt, map_location='cpu')
